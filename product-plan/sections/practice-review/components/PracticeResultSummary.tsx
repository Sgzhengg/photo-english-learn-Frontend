...
import type { PracticeReviewProps as Props, PracticeResult } from '../types'
...
function AccuracyRing({ accuracy, size = 120 }: { accuracy: number; size?: number }) {
  ...
  const getColor = () => {
    if (accuracy >= 0.9) return 'from-lime-400 to-emerald-500'
    if (accuracy >= 0.7) return 'from-blue-400 to-indigo-500'
    if (accuracy >= 0.5) return 'from-amber-400 to-orange-500'
